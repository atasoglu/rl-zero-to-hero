...
MUJOCO_ENVS = ["HalfCheetah-v4", "Ant-v4", "Hopper-v4"]


def run_episode(agent: DDPGAgent, env, train: bool = True) -> float:
    obs, _ = env.reset()
    agent.reset_noise()
    total_reward = 0.0
    done = False
...
def train(agent: DDPGAgent, env, n_episodes: int, render_env=None, render_interval: int = 0) -> list[float]:
    rewards = []
...
def evaluate(agent: DDPGAgent, env, n_episodes: int = 10) -> float:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--noise-sigma", type=float, default=0.2, help="OU noise standard deviation")
    ...
    plot_rewards(rewards, title=f"DDPG on {args.env}")
