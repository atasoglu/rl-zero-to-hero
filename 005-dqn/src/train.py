...
def run_episode(agent: DQNAgent, env, train: bool = True) -> float:
    obs, _ = env.reset()
    total_reward = 0.0
    done = False
    while not done:
...
def train(agent: DQNAgent, env, n_episodes: int, render_env=None, render_interval: int = 0) -> list[float]:
    rewards = []
...
def evaluate(agent: DQNAgent, env, n_episodes: int = 10) -> float:
    saved_eps = agent.epsilon
    agent.epsilon = 0.0
...
if __name__ == "__main__":
    ...
    plot_rewards(rewards, title=f"DQN on {args.env}")
