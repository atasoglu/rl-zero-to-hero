...
class HERSACAgent:
    def __init__(
        ...
        self.obs_size = obs_size  # raw observation dimension (without goal)
        ...
        obs_dim = obs_size + goal_dim  # policy input = obs + desired_goal
        ...
        self.buffer = HERReplayBuffer(buffer_size, obs_dim, action_dim, k=her_k)
    ...
    def obs_to_flat(self, obs_dict: dict) -> np.ndarray:
        return np.concatenate([obs_dict["observation"], obs_dict["desired_goal"]])

    def select_action(self, obs_flat: np.ndarray, explore: bool = True) -> np.ndarray:
    ...
    def push_step(self, obs_flat, action, reward, next_obs_flat, done, next_achieved_goal) -> None:
        self.buffer.push_step(obs_flat, action, reward, next_obs_flat, done, next_achieved_goal)

    def finalize_episode(self, compute_reward_fn) -> None:
        self.buffer.finalize_episode(self.obs_size, compute_reward_fn)
    ...
    def update(self) -> None:
        ...
        with torch.no_grad():
            ...
            min_q_target = torch.min(q1_t, q2_t) - self.alpha * next_log_probs
