...
def run_episode(agent: PERAgent, env, train: bool = True) -> float:
    obs, _ = env.reset()
    total_reward = 0.0
    done = False
    while not done:
...
def train(agent: PERAgent, env, n_episodes: int, render_env=None, render_interval: int = 0) -> list[float]:
    rewards = []
    bar = tqdm(range(n_episodes), desc="Training", unit="ep")
    for ep in bar:
        agent.anneal_beta(1.0 / n_episodes)
...
def evaluate(agent: PERAgent, env, n_episodes: int = 10) -> float:
    saved_eps = agent.epsilon
    agent.epsilon = 0.0
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env", default="LunarLander-v3",
                        choices=["CartPole-v1", "LunarLander-v3"] + ATARI_ENVS,
                        help="gym environment (Atari envs require ale-py + ROMs)")
    ...
    parser.add_argument("--per-alpha", type=float, default=0.6, help="PER: priority exponent (0=uniform, 1=fully greedy)")
    parser.add_argument("--per-beta-start", type=float, default=0.4, help="PER: IS weight exponent at episode 0")
    parser.add_argument("--per-beta-end", type=float, default=1.0, help="PER: IS weight exponent at final episode")
    ...
    render_env = (make_atari_env(args.env, render_mode="human") if is_atari
                  else make_env(args.env, render_mode="human")) if args.render_interval else None
    rewards = train(agent, env, args.episodes, render_env=render_env, render_interval=args.render_interval)
    if render_env:
        render_env.close()

    eval_env = make_atari_env(args.env) if is_atari else make_env(args.env)
    ...
    plot_rewards(rewards, title=f"PER on {args.env}")

    env.close()
    eval_env.close()

    if args.watch > 0:
        render_env = make_atari_env(args.env, render_mode="human") if is_atari else make_env(args.env, render_mode="human")
