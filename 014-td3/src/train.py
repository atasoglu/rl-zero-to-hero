...
MUJOCO_ENVS = ["HalfCheetah-v4", "Ant-v4", "Hopper-v4", "Walker2d-v4"]


def run_episode(agent: TD3Agent, env, train: bool = True) -> float:
    obs, _ = env.reset()
    total_reward = 0.0
    done = False
    while not done:
...
def train(agent: TD3Agent, env, n_episodes: int, render_env=None, render_interval: int = 0) -> list[float]:
    rewards = []
...
def evaluate(agent: TD3Agent, env, n_episodes: int = 10) -> float:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy-noise", type=float, default=0.2, help="target policy smoothing noise std")
    parser.add_argument("--noise-clip", type=float, default=0.5, help="target smoothing noise clipping range")
    parser.add_argument("--policy-delay", type=int, default=2, help="actor update frequency relative to critic")
    parser.add_argument("--explore-noise", type=float, default=0.1, help="exploration noise std during training")
    ...
    plot_rewards(rewards, title=f"TD3 on {args.env}")
