...
def run_episode(agent: A2CAgent, env, train: bool = True) -> float:
    obs, _ = env.reset()
    total_reward = 0.0
    done = False
    while not done:
...
def train(agent: A2CAgent, env, n_episodes: int, render_env=None, render_interval: int = 0) -> list[float]:
    rewards = []
...
def evaluate(agent: A2CAgent, env, n_episodes: int = 10) -> float:
...
if __name__ == "__main__":
    ...
    plot_rewards(rewards, title=f"A2C on {args.env}")
