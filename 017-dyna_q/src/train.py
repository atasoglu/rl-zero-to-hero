...
def run_episode(agent: DynaQAgent, env, train: bool = True) -> float:
    state, _ = env.reset()
    total_reward = 0.0
    done = False
    while not done:
...
def train(agent: DynaQAgent, env, n_episodes: int, render_env=None, render_interval: int = 0) -> list[float]:
    rewards = []
...
def evaluate(agent: DynaQAgent, env, n_episodes: int = 100) -> float:
    saved_eps = agent.epsilon
    agent.epsilon = 0.0
...
if __name__ == "__main__":
    ...
    # Environment
    parser.add_argument("--env", default="FrozenLake-v1", choices=["FrozenLake-v1", "Taxi-v3"],
                        help="gym environment")
    parser.add_argument("--slippery", action=argparse.BooleanOptionalAction, default=False,
                        help="FrozenLake: stochastic transitions (--no-slippery for deterministic)")
    ...
    parser.add_argument("--n-planning", type=int, default=10,
                        help="simulated planning steps per real step (0 = pure Q-learning)")
    ...
    label = f"n_planning={args.n_planning}" if args.n_planning > 0 else "Q-learning (no planning)"
    print(f"{args.env} | {label} | episodes: {args.episodes} | eval mean reward: {mean_reward:.3f}")

    plot_rewards(rewards, title=f"Dyna-Q on {args.env} ({label})")
