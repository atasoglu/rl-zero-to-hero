...
def collect_rollout(agent: RNDAgent, env, n_steps: int) -> tuple[dict, float, int]:
    obs_buf = []
    actions_buf, log_probs_buf = [], []
    v_ext_buf, v_int_buf = [], []
    ...
    for _ in range(n_steps):
        action, log_prob, v_ext, v_int = agent.select_action(obs)
        ...
        v_ext_buf.append(v_ext)
        v_int_buf.append(v_int)
        ext_rewards_buf.append(float(reward))
        dones_buf.append(float(done))
        total_ext += float(reward)
    ...
    int_rewards = agent.compute_intrinsic_rewards(obs_arr)

    return {
        "obs": obs_arr,
        "actions": np.array(actions_buf),
        "log_probs": np.array(log_probs_buf),
        "v_ext": np.array(v_ext_buf),
        "v_int": np.array(v_int_buf),
        "ext_rewards": np.array(ext_rewards_buf),
        "int_rewards": int_rewards,
        "dones": np.array(dones_buf),
    }, total_ext, max(n_episodes, 1)


def train(agent: RNDAgent, env, n_updates: int, n_steps: int) -> list[float]:
    rewards_log = []
    ...
    for _ in bar:
        ...
        rewards_log.append(avg_ext)
        if len(rewards_log) >= 10:
            avg10 = sum(rewards_log[-10:]) / 10
...
if __name__ == "__main__":
    ...
    parser.add_argument("--int-coef", type=float, default=1.0, help="intrinsic reward coefficient")
    parser.add_argument("--ext-coef", type=float, default=2.0, help="extrinsic reward coefficient")
    ...
    plot_rewards(rewards, title=f"RND+PPO on {args.env}")
