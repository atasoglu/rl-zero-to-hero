...
def run_episode(agent: SARSAAgent, env, train: bool = True) -> float:
    state, _ = env.reset()
...
def train(agent: SARSAAgent, env, n_episodes: int, render_env=None, render_interval: int = 0) -> list[float]:
    rewards = []
...
def evaluate(agent: SARSAAgent, env, n_episodes: int = 100) -> float:
    saved_eps = agent.epsilon
    agent.epsilon = 0.0
...
if __name__ == "__main__":
    ...
    # Environment
    parser.add_argument("--env", default="FrozenLake-v1", choices=["FrozenLake-v1", "CliffWalking-v0"], help="gym environment")
    ...
    plot_rewards(rewards, title=f"SARSA on {args.env}")
